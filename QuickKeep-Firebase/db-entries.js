...
async function saveEntry(uid, plan, { url, title, note, folderId = null, tags = null }) {
  ...
  if (plan === 'free') {
    ...
    if (snap.size >= FREE_LIMIT) {
      throw { code: 'LIMIT_REACHED', upgrade: true,
  ...
  if (!existing.empty) {
    ...
    await ref.update({ note: note || '', updatedAt: firebase.firestore.FieldValue.serverTimestamp() });
...
async function deleteFolder(folderId) {
  ...
  const snap = await db.collection('entries').where('folderId', '==', folderId).get();
  const batch = db.batch();
...
async function localAddEntry({ url, title, note }) {
  ...
  if (existing > -1) {
    ...
    entries.push({
      ...
      url, title: title || 'Untitled', note: note || '',
      savedAt:   Date.now(),
    });
  }
